...
async function loadNormalRequests(onRecovery) {
  ...
  // 1. ถ้าข้อมูล valid ให้ส่งคืนไปใช้งานได้เลย
  if (stored.status === 'valid') {
    return stored.requests;
  }

  // 2. ถ้าข้อมูล invalid (ข้อมูลเสีย/ไม่ตรง schema) ให้เรียก onRecovery
  if (stored.status === 'invalid' && onRecovery) {
    onRecovery(stored.reason);
  }

  // 3. ดึงข้อมูล seed ใหม่มาเขียนทับข้อมูลที่พังหรือข้อมูลที่ว่างเปล่า
...
export async function getRequests(options = {}) {
  ...
  // เปลี่ยนบรรทัดนี้เพื่อส่ง options.onRecovery ต่อไปให้ loadNormalRequests
