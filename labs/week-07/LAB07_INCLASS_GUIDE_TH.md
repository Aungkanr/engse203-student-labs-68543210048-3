...
- [x] API เปิดที่ 3001 · เปิดเบราว์เซอร์ไปที่ `http://localhost:3001` เห็น JSON
- [x] React เปิดที่ 5173 · เห็นหน้า Dashboard (ยังโหลดจาก localStorage อยู่)
- [x] เปิด DevTools (F12) ค้างไว้ที่แท็บ Console
...
- [x] เห็น CORS error ใน Console ด้วยตาตัวเอง
- [x] เห็น log ใน terminal ของ API ว่าคำขอมาถึงจริง
- [x] ยิง Postman แล้วได้ 200 ปกติ (ไม่ติด CORS)
- [x] อธิบายได้ว่า**ใครเป็นคนบล็อก**
...
- [x] CORS error หายไปจาก Console
- [x] Network tab เห็น header `Access-Control-Allow-Origin`
- [x] แก้ `PORT` ใน `.env` แล้วพอร์ตเปลี่ยนตามจริง
- [x] ไม่มีเลข `3001` ฝังอยู่ใน `server.js` แล้ว
...
- [x] Dashboard โหลดข้อมูลจาก API ได้ (ยืนยันด้วย Network tab)
- [x] เพิ่ม / ลบ คำร้องได้ และข้อมูลอยู่ที่เซิร์ฟเวอร์จริง
- [x] `REQ-999` ขึ้น "ไม่พบคำร้อง" ไม่ใช่หน้า error
- [x] **ปิด API แล้วข้อมูลโหลดไม่ขึ้น** (พิสูจน์ว่าไม่ได้อ่าน localStorage)
- [x] **ไม่ได้แก้ไฟล์ component แม้แต่ไฟล์เดียว**
...
- [x] ปิด API แล้วหน้าจอขึ้นข้อความที่ผู้ใช้เข้าใจ ไม่ใช่หน้าขาวหรือข้อความ error ดิบ
- [x] มีปุ่มให้ลองใหม่ และกดแล้วทำงานจริง
- [x] ทดสอบครบทั้ง 4 สถานะ
...
- [x] `node check-week07.mjs --inclass` ผ่าน **25/25**
- [x] เปิดสองหน้าต่างแล้วแอปทำงานได้ครบ — ดู เพิ่ม ลบ
- [x] ปิด API แล้วขึ้นข้อความที่ผู้ใช้เข้าใจ
- [x] **ยืนยันว่าไม่ได้แก้ component เลยสักไฟล์**
- [x] `git add -A && git commit -m "LAB07 in-class: เชื่อม React กับ API"` แล้ว
