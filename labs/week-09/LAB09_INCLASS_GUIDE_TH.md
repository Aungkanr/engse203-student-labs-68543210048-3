...
- [x] วาง `playground-seed.sql` แล้วเห็นตาราง 5 แถว
- [x] เขียน `SELECT` เลือกคอลัมน์ที่ต้องการได้
- [x] กรองด้วย `WHERE` ได้ทั้งเงื่อนไขเดียวและหลายเงื่อนไข
- [x] ใช้ `LIKE` ค้นจากคำบางส่วนได้
- [x] เรียงด้วย `ORDER BY` ได้ทั้ง `ASC` และ `DESC`
...
- [x] วาดตารางทั้งสองพร้อม PK และ FK ได้
- [x] อธิบายได้ว่าทำไมต้องแยก `users` ออกมา
- [x] ระบุได้ว่าคอลัมน์ไหนควรมี constraint อะไร
...
- [x] มีไฟล์ `campus.db` ที่มีทั้ง 2 ตาราง
- [x] `users` มี 4 คน · `requests` มี 5 รายการ
- [x] เปิดการตรวจ Foreign Key แล้ว
...
- [x] เพิ่ม แก้ไข และลบคำร้องได้
- [x] เห็นว่า `DEFAULT` เติมค่าให้เองเมื่อไม่ได้ระบุ
- [x] **ลองทำผิดทั้ง 3 แบบแล้วเห็น error จริง**
...
- [x] เขียน `JOIN` ได้ครบทั้ง 4 ข้อ
- [x] อธิบายได้ว่า `ON` ทำหน้าที่อะไร
- [x] **เห็นว่าผลลัพธ์ข้อ ④ เหมือนข้อมูลที่ API ส่งให้ React**
...
- [x] `check-week09.mjs --inclass` ผ่าน **11/11**
- [x] มีไฟล์ `campus.db` ที่ query ได้จริง
- [x] เขียน SELECT · WHERE · ORDER BY · INSERT · UPDATE · DELETE · JOIN เป็น
- [x] **ลองทำผิดแล้วเห็น constraint ปฏิเสธจริง**
- [x] commit แล้ว: `git add -A && git commit -m "LAB09 in-class: SQL และฐานข้อมูลเชิงสัมพันธ์"`
