...
- [x] เปิด server ได้โดยไม่มี error
- [x] ยิง `GET /api/requests` แล้วเห็นข้อมูลจากฐานข้อมูล
- [x] เข้าใจว่า `all` `get` `run` ต่างกันอย่างไร
...
- [x] รันจากสองโฟลเดอร์ที่ต่างกัน แล้วเปิดฐานข้อมูล**ตัวเดียวกัน**ได้ทั้งคู่
- [x] เข้าใจว่าทำไม `./` ถึงไม่น่าเชื่อถือ
...
- [x] `GET /api/requests` คืน array ที่มี `requesterName`
- [x] `?status=pending` กรองได้
- [x] `GET /api/requests/REQ-001` ได้ 200 · id ที่ไม่มีได้ 404
- [x] **เปิดแอป React แล้ว Dashboard แสดงข้อมูลจากฐานข้อมูล**
...
- [x] CRUD ครบทั้ง 5 endpoint
- [x] checker Week 10 `--inclass` ผ่าน 20/20
- [x] **checker Week 07 ยังผ่าน 36/36**
- [x] **ยืนยันว่าไม่ได้แก้ไฟล์อื่นนอกจาก `requestService.js`**
...
- [x] `check-week10.mjs --inclass` ผ่าน **20/20**
- [x] `check-week07.mjs` ผ่าน **36/36**
- [x] แก้แค่ `requestService.js` ไฟล์เดียว
- [x] ปิดเซิร์ฟเวอร์แล้วเปิดใหม่ ข้อมูลยังอยู่
- [x] commit: `git add -A && git commit -m "LAB10 in-class: เชื่อม Node เข้ากับฐานข้อมูล"`
