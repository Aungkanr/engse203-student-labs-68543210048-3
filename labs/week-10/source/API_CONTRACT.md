...
| ตาราง `users` | ชนิด | คำอธิบาย |
|---|---|---|
| `id` | INTEGER (PK) | รหัสผู้ใช้ สร้างอัตโนมัติ |
| `name` | TEXT | ชื่อผู้ใช้ |
| `department` | TEXT | แผนก |
| `email` | TEXT (UNIQUE) | อีเมล (ห้ามซ้ำ) |

| ตาราง `requests` | ชนิด | คำอธิบาย |
|---|---|---|
| `id` | TEXT (PK) | รหัสคำร้อง เช่น REQ-001 |
| `requester_id` | INTEGER (FK) | ชี้ไปที่ `users.id` |
| `request_type` | TEXT | ประเภทคำร้อง (มี CHECK constraint) |
| `location` | TEXT | สถานที่ |
| `details` | TEXT | รายละเอียด |
| `priority` | TEXT | ความสำคัญ (normal, urgent) |
| `status` | TEXT | สถานะ (pending, in-progress, completed) |

### ข้อสังเกตเรื่องรูปแบบ
โครงสร้างในฐานข้อมูลไม่เหมือนรูปแบบที่ API ส่งออก:
- ฐานข้อมูลเก็บ `requester_id` (ตัวเลข) เพื่อหลีกเลี่ยงการเก็บข้อมูลชื่อซ้ำซ้อน (Data Redundancy)
- API ส่งคืน `requesterName` (ข้อความ) เพราะ Frontend ต้องการแสดงผลชื่อทันที
- ชั้น Service (`requestService.js`) ทำหน้าที่เป็นตัวกลางแปลงข้อมูลด้วยคำสั่ง `JOIN` และ `AS` ก่อนส่งกลับไป

### พฤติกรรมของ POST (การสร้างคำร้อง)
**พฤติกรรมนี้สำคัญ:** หากมีการส่งคำร้องที่มี `requesterName` ใหม่ที่ยังไม่เคยมีในฐานข้อมูล **API จะทำการสร้างผู้ใช้ (User) คนใหม่ลงในตาราง `users` ให้อัตโนมัติทันที** ก่อนที่จะสร้างคำร้อง (Request) เพื่อป้องกันข้อผิดพลาดจาก Foreign Key
